...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Kein Fahrzeug zum abschließen in der umgebung.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Du ~r~verriegelst~s~ dein ~y~'..vehicleLabel..'~s~.')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Du ~g~entriegelst~s~ dein ~y~'..vehicleLabel..'~s~.')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
					...
					if notowned == #carstrie then
						ESX.ShowNotification("Kein Fahrzeug in der umgebung.")
					end	
				end, plate)
			end			
		end
